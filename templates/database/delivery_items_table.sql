...
END;
$function$;
-- Updated function to create delivery with individual item UUIDs instead of group_id
CREATE OR REPLACE FUNCTION public.create_delivery_with_items(
  p_admin_uuid uuid,
  p_company_uuid uuid,
  p_warehouse_uuid uuid,
...
BEGIN
  ...
  LOOP
    ...
    -- Collect item UUIDs (the keys are the individual item UUIDs)
    v_item_uuids := array_append(v_item_uuids, v_item_key::uuid);
  END LOOP;
  
  -- Validate that all item UUIDs exist
  IF v_item_uuids IS NOT NULL AND array_length(v_item_uuids, 1) > 0 THEN
    -- Check each item exists
    FOR i IN 1..array_length(v_item_uuids, 1)
    LOOP
      PERFORM 1 FROM inventory_items 
      WHERE uuid = v_item_uuids[i]
        AND company_uuid = p_company_uuid;
      
      IF NOT FOUND THEN
        RETURN jsonb_build_object(
          'success', false,
          'error', 'Item UUID ' || v_item_uuids[i] || ' not found'
        );
      END IF;
    END LOOP;
  END IF;
  ...
  -- Update inventory item status to 'ON_DELIVERY' for ONLY the selected items
  IF v_item_uuids IS NOT NULL AND array_length(v_item_uuids, 1) > 0 THEN
    UPDATE inventory_items 
    SET 
      status = 'ON_DELIVERY',
...
-- Update the update_delivery_with_items function to handle individual item UUIDs
...
DECLARE
  ...
  v_timestamp text;
  v_result jsonb;
  v_new_item_uuids uuid[];
  v_old_item_uuids uuid[];
  v_items_to_add uuid[];
...
BEGIN
  ...
  -- Extract new item UUIDs (the keys are the individual item UUIDs)
  FOR v_item_key IN SELECT jsonb_object_keys(p_inventory_items)
  LOOP
    v_new_item_uuids := array_append(v_new_item_uuids, v_item_key::uuid);
  END LOOP;
  
  -- Extract old item UUIDs from existing structure
  FOR v_item_key IN SELECT jsonb_object_keys(v_delivery_record.inventory_items)
  LOOP
    v_old_item_uuids := array_append(v_old_item_uuids, v_item_key::uuid);
  END LOOP;

  -- Handle null arrays
  v_new_item_uuids := COALESCE(v_new_item_uuids, ARRAY[]::uuid[]);
  v_old_item_uuids := COALESCE(v_old_item_uuids, ARRAY[]::uuid[]);

  -- Find items to add (in new but not in old)
  SELECT array_agg(item_uuid) INTO v_items_to_add
  FROM unnest(v_new_item_uuids) AS item_uuid
  WHERE item_uuid != ALL(v_old_item_uuids);

  -- Find items to remove (in old but not in new)
  SELECT array_agg(item_uuid) INTO v_items_to_remove
  FROM unnest(v_old_item_uuids) AS item_uuid
  WHERE item_uuid != ALL(v_new_item_uuids);

  -- Handle null arrays
  v_items_to_add := COALESCE(v_items_to_add, ARRAY[]::uuid[]);
  v_items_to_remove := COALESCE(v_items_to_remove, ARRAY[]::uuid[]);

  -- Validate that all new item UUIDs exist
  FOR v_item_key IN SELECT jsonb_object_keys(p_inventory_items)
  LOOP
    PERFORM 1 FROM inventory_items 
    WHERE uuid = v_item_key::uuid
      AND company_uuid = v_delivery_record.company_uuid;
    
    IF NOT FOUND THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Item UUID ' || v_item_key || ' not found'
      );
    END IF;
  END LOOP;
...
-- Update the status function to work with individual item UUIDs
...
BEGIN
  ...
  -- Extract item UUIDs (the keys are the individual item UUIDs)
  FOR v_item_key IN SELECT jsonb_object_keys(v_delivery_record.inventory_items)
  LOOP
    v_item_uuids := array_append(v_item_uuids, v_item_key::uuid);
  END LOOP;
  ...
  -- Update inventory item status for ONLY the specific items in the delivery
  IF v_item_uuids IS NOT NULL AND array_length(v_item_uuids, 1) > 0 THEN
    UPDATE inventory_items 
    SET 
  ...
  IF p_status = 'DELIVERED' THEN
    SELECT public.create_warehouse_inventory_from_delivery(
      ...
      p_delivery_uuid,
      v_item_uuids, -- Pass item UUIDs instead of group IDs
...
-- Updated function to create warehouse inventory from delivery with individual item UUIDs
CREATE OR REPLACE FUNCTION public.create_warehouse_inventory_from_delivery(
  p_warehouse_uuid uuid,
  p_delivery_uuid uuid,
  p_item_uuids uuid[], -- Changed from group_ids to item_uuids
  p_inventory_items jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
...
-- Update get_delivery_details to work with individual item UUIDs
...
BEGIN
  ...
  IF v_item_uuids IS NOT NULL AND array_length(v_item_uuids, 1) > 0 THEN
    ...
    WHERE ii.uuid = ANY(v_item_uuids);
  ELSE
    v_inventory_items := '[]'::jsonb;
  END IF;
